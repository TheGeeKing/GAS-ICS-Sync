...
function createEvent(event, calendarTz) {
  ...
  if (ignoreModifiedProperty) {
    let ignoreModifiedPropertyValues = {}; // We save the values to add them back after hashing comparison
    for (property of ignoreModifiedProperty) {
      if (event.hasProperty(property)) {
        ignoreModifiedPropertyValues[property] = event.getFirstPropertyValue(property);
  ...
  if (ignoreModifiedProperty && ignoreModifiedPropertyValues) {
    for (property in ignoreModifiedPropertyValues) {
      event.addPropertyWithValue(property, ignoreModifiedPropertyValues[property]); // We add back the properties that were ignored
    }
  }
